...
void FibonacciHeap::insert(FiboNode *node)
{
	...
	//cout << "insert " << m_nNode << endl;
	current_node_id.insert((node->key).index);
	m_nNode++;
}
...
FiboNode* FibonacciHeap::extractMin()
{
	...
	if (minheap!=NULL)
	{
		...
		current_node_id.erase((z->key).index);
		m_nNode--;
	//	KEYTYPE tk = z->key;
	//	delete z;
		return z;
...
{
	size_t i,Dn = 1+(size_t)(log((float)m_nNode)/log(0.5*(1.0+sqrt(5.0))));
